...
def find_contamination_bottom_and_top(image, starting_point, position=0, num_rows=50, shouwDebug=False):
    ...
    # Calculate the line_first_gradient of the line values
    line_first_gradient = np.gradient(line_values)

    # Find all local maximums and minimums in the line_first_gradient
    maxs, _ = find_peaks(line_first_gradient, prominence=0) 
    ...
    if shouwDebug:
        ...
        plt.title('Gradient Analysis of Vertical Line Profile', fontsize=22)
        plt.legend()
